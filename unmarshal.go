...
func (s *SessionDescription) Unmarshal(value []byte) error {
	...
	// s.MediaDescriptions = make([]MediaDescription, 0, stats.b)
...
func (l *lexer) unmarshalConnectionInformation() (c ConnectionInformation, err error) {
	...
	if !anyOf(c.NetworkType, kIn) {
		return c, fmt.Errorf("%w `%v`", errSDPInvalidValue, c.NetworkType)
	}
	...
	if !anyOf(c.AddressType, kIp4, kIp6) {
		return c, fmt.Errorf("%w `%v`", errSDPInvalidValue, c.AddressType)
	}
...
func unmarshalBandwidth(value []byte) (*Bandwidth, error) {
	...
	if experimental {
		...
		return nil, fmt.Errorf("%w `%v`", errSDPInvalidValue, typ)
	}

	bandwidth, ok := parseUint(value[i+1:], 64)
	if !ok {
		return nil, fmt.Errorf("%w `%v`", errSDPInvalidNumericValue, value[i+1:])
	}
...
func unmarshalMediaDescription(l *lexer) (stateFn, error) {
	...
	if i == -1 {
		...
		if !ok {
			return nil, fmt.Errorf("%w `%v`", errSDPInvalidValue, field[i+1:])
		}
	...
	if !ok {
		return nil, fmt.Errorf("%w `%v`", errSDPInvalidPortValue, field[:i])
	}
	...
	newMediaDesc.MediaName.Protos = make([][]byte, 0, 2)
	for pos, off := 0, 0; off < len(field); off += pos + 1 {
		if pos = bytes.IndexRune(field[off:], '/'); pos == -1 {
			pos = len(field)
		}
		proto := field[off:pos]
